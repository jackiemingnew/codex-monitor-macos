...
没有配置本机 token 时，应用会自动读取 `https://codexradar.com/current.json` 公开 summary。有 token 时会读取 `https://codexradar.com/api/v1/current`。token 只保存在当前 Mac 的应用支持目录或设置页写入的本机配置中，不应提交到仓库。
...
Without a local token, the app reads the public summary at `https://codexradar.com/current.json`. With a token, it reads `https://codexradar.com/api/v1/current`. The token is stored only in the current Mac's application support data or local settings path, and should never be committed to the repository.
