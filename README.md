...
没有配置本机 token 时，应用会自动读取 `https://codexradar.com/current.json` 公开 summary。有 token 时会读取 `https://codexradar.com/api/v1/current`；token 无效或 API 暂不可用时会明确提示并降级到公开 summary。应用仍在北京时间 08:20 和 14:20 自动刷新，打开 Radar 时也会校验超过 30 分钟的缓存。token 只保存在当前 Mac 的应用支持目录或设置页写入的本机配置中，不应提交到仓库。
...
Without a local token, the app reads the public summary at `https://codexradar.com/current.json`. With a token, it reads `https://codexradar.com/api/v1/current`; an invalid token or temporary API failure is reported before falling back to the public summary. Scheduled refreshes remain at 08:20 and 14:20 Beijing time, and opening Radar also validates caches older than 30 minutes. The token is stored only in the current Mac's application support data or local settings path, and should never be committed to the repository.
