...
项目当前支持本机 Codex、Skill Insights、Codex Radar、CLIProxyAPI / CPA Manager Plus、NewAPI 和 Sub2API 多类数据源，适合需要长期观察 Codex 使用量、Skill 证据、账号额度、模型动态和远程代理面板状态的用户。
...
- 顶部 HUD 可水平或垂直拖动并记住位置；右键可返回主屏默认中心位置。
- 使用全局快捷键 `⌃⌥⌘M` 可显示或隐藏详情页，无需唤出隐藏的菜单栏界面。
- 详情页使用多 tab 模式：`Codex`、`性能`、`Skills`、`Codex Radar`、`CLIProxyAPI`、`NewAPI`、`Sub2API`。
- `性能` tab 显示 Codex / ChatGPT、Safari 主进程组、最热 WebKit 内容进程和 WindowServer 的 CPU、内存与 30 秒峰值。
...
- 现有额度响应中的可用重置次数，以及今日、7 天、30 天的 API 标准单价等值费用。
- 5 小时和 7 天额度的线性消耗速度、预计耗尽提示、数据来源与最后成功更新时间。

费用是基于本地 Codex 日志和标准 API 单价的等值估算，不是 ChatGPT/Codex 订阅账单；不包含 Priority、区域溢价或工具调用费。首次完整历史扫描期间显示“回填中”，不会把局部小计冒充窗口总价；未知模型不猜价。扫描对齐 CodexBar 的首条会话元数据、fork lineage 与跨文件 Token 行去重语义，并在独立串行 utility 队列中每次最多执行一个 8 MiB / 50ms CPU / 250ms 墙钟切片；后续刷新从完整行检查点继续，完整语料追平后才原子发布快照。完成后页脚 Token 与费用使用同一份结果。打开详情页不会触发扫描，低电量或严重温控会暂停，暖缓存不会重复读取 JSONL。CLI、Node 和 JSON 的既有 Token 字段不变。

设置中的“自适应低能耗刷新”在性能门槛通过后对新安装默认开启；已有安装会保留原有选择，没有选择记录时继续使用保守的固定刷新。开启后，本地和远端刷新会依据对应详情页是否实际展开、Codex 是否运行、低电量模式和温度压力自动节流；折叠 HUD 只展示缓存状态，不会进入前台刷新档位。文件变化、手动刷新和额度恢复边界仍会及时响应。打开详情页时，新鲜数据不会重复启动扫描、网络请求或子进程。
...
### 个人 Pro 官方网页 Analytics

`Codex` 详情页任务列表下方提供 48pt 的“官方数据 · 7天”入口，显示当前个人
Pro 账号在 Codex Analytics 网页中可见的 Turns、Skills 和 Plugin calls。
独立的 `Analytics` 详情页使用原生 Swift Charts 绘制最近 7 天的 Turns
（By model / By surface）和 Skills used 堆叠面积图，并在固定 HUD 内纵向滚动。
官方网页质量与本地任务、额度、Token 历史和 RUN / IDLE 完全隔离。

首次使用时点击区块中的“网页登录”，在 Codex Monitor 打开的可见窗口中完成
ChatGPT 登录。登录会话保存在本应用专属的 WebKit 网站数据仓库中，重启后可以
继续复用；应用不读取 Chrome / Safari Cookie，也不保存密码、原始网页或
Analytics 快照。网页窗口提供“清除登录”，只删除本应用的 WebKit 会话，不影响
其他浏览器。登录成功后自动读取一次，成功数据在内存中缓存 30 分钟；缓存过期后
仅在再次打开详情页或手动刷新时更新，不运行后台定时器。
应用启动、Codex 详情和本地 Token 模式都不会创建 WebKit；只有显式进入官方
Analytics 或打开网页登录窗口时才创建。官方页面和网页登录窗口都离开 30 分钟后
会释放 WKWebView，但保留同一个网站数据仓库，因此下次仍可复用应用内登录态。
若 ChatGPT 登录流程停留在主页，点击网页窗口顶部的“重新载入网页”即可回到
Analytics；该按钮始终导航到精确的官方 Analytics 地址。

个人 Pro 页面没有稳定的公开 Analytics API。应用只读取页面中用户可见的三个
KPI 以及模型、Surface、Skills 图表 Tooltip，不调用或回放内部网络接口。三个
图表必须覆盖同一组完整 7 天；模型 / Surface 合计必须等于 Turns，Skills 图表
合计必须等于 Skills used，才会标记 `COMPLETE`。不完整但经过校验的日期仍可在
原生图表中查看，并标记 `PARTIAL`；缺失日期不会被补成 0。页面结构变化、登录
失效或范围选择失败会显示 `PARTIAL`、`STALE` 或 `UNAVAILABLE`，不会用本地
数据冒充官方网页结果。原生图表不创建第二个 WebKit，网页登录窗口仍用于登录和
核对官方来源。

Analytics 页还提供“本地 Token”模式，直接读取完整发布的本机成本快照，按今日、
7 天或 30 天显示模型 Token 趋势和排行；选择图表不会新增 JSONL 扫描、网络请求或
WebKit。缓存输入是输入的子集，不会重复计入总 Token；未定价模型仍计入 Token 和占比。

### 后台性能诊断

设置中的“后台性能监控”默认关闭。性能详情可见时每 5 秒采样；详情收起后只有开启该选项才每 60 秒采样，低电量模式或严重/危急温度压力下放慢到 5 分钟。启用后台监控时，有界历史写入 `~/Library/Logs/CodexMonitor/performance-samples.jsonl`。日志最多 4 MiB，并保留一个轮转备份；只记录时间、CPU、RSS、进程数、候选 PID 和系统内存比例，不记录 URL、窗口标题、命令参数或对话内容。

Safari 的 WebKit XPC 进程无法通过轻量系统数据可靠映射到具体标签，因此页面显示的是“最热 WebKit 内容候选”，归属明确标记为未验证。关闭或刷新疑似标签后，如果同一 PID 消失且 CPU/内存回落，可作为强对照证据。WindowServer CPU 仅代表窗口合成压力和掉帧风险，不是实测 FPS；应用不会为伪造 FPS 请求录屏或辅助功能权限。

### Skill Insights

`Skills` 详情页提供一个只读、低频的最近 7 天 Skill 周报，用确定性证据帮助判断已启用 Skill 是否真正使用、可能漏触发或误触发，以及关闭 Skill 是否出现值得复测的需求信号。

可显示：

- 当前启用和关闭 Skill 数量，以及启用目录中 `name + description` 的近似上下文 Token 成本。
- `DIRECT`、`STRONG`、`INFERRED` 和 `SHADOW` 证据，以及疑似漏触发、疑似误触发和被现有能力替代的候选。
- 保留、继续观察、继续关闭、建议复测和恢复候选等保守建议。
- `COMPLETE`、`PARTIAL` 或 `UNAVAILABLE` 数据完整度、最后分析时间和增量扫描性能。
- Markdown 周报和机器可读 JSON 导出。

启用、关闭和目录描述来自本机 Codex app-server 的只读 `skills/list` 结果，因此沿用 Codex 自身对 `config.toml`、插件启用状态、作用域和路径优先级的判断，不会把未激活的插件缓存计入当前目录。若本地接口不可用，应用会退回 frontmatter 文件扫描并明确标记 `PARTIAL`，不会把回退结果声明为完整真值。

Skill Insights 可在设置的 `Codex 数据` 中独立关闭。关闭后会隐藏 `Skills` tab、释放分析服务和周定时器，并停止 Skill JSONL 读取与派生数据库写入；Token、Quota、Context 和 Delta 主监测不受影响。

启用时，App 启动后只检查周级计划，滚动每 7 天最多自动分析一次，也可手动执行“分析最近 7 天”。手动分析从已保存的完整行 offset 继续，不会清空检查点。分析只增量读取变化文件，不由实时 Token 刷新、文件变化或监听事件触发；低电量模式或严重/临界热状态会延后本周自动分析，且不会增加重试频率。目录请求只经本机 stdio 发送给 Codex，不调用模型、embedding 或外部分类服务，不向外部发送 Session/Skill 数据，也不会修改 `~/.codex/config.toml` 或任何 Skill 文件。

扫描使用单线程后台任务、块级 JSONL reader 和持久 SQLite 连接。单次最多读取 2GiB、使用 15 秒进程 CPU 或运行 30 秒；单行解析上限为 256KiB，相关或无法判定的超限行会明确传播 `PARTIAL`。达到任一运行上限会在完整行检查点处显示 `PARTIAL` 并由下一次手动或周级分析继续。未变化文件不会重复读取；启用/关闭状态只在报告阶段重新分类中性证据，不会触发 JSONL 重扫。

派生数据保存在 `~/Library/Application Support/CodexNotch/skill-observations.sqlite`，默认保留 30 天。数据库不保存完整 Prompt、助手回答、reasoning 或完整工具输入输出。相关 Session Token 只是整个 Session 的参考总量；逐 Skill Token 始终为 `UNAVAILABLE`，不会把 Session Token 归因给某个 Skill。详细指标和诊断边界见 [`docs/METRIC_DEFINITIONS.md`](docs/METRIC_DEFINITIONS.md) 与 [`docs/DIAGNOSTICS.md`](docs/DIAGNOSTICS.md)。
...
- `Codex`：本机 Codex 固定/自适应刷新、后台性能监控、额度来源、任务范围和历史用量显示。
...
只记录额度、reset、来源、选择原因和固定功能状态，不记录任务内容、任意键盘输入、账号或凭证。字段说明见
[`docs/DIAGNOSTICS.md`](docs/DIAGNOSTICS.md)。

打印一次当前性能快照，或读取最近 200 条后台性能历史：

```bash
.build/release/CodexNotch --print-performance-snapshot
.build/release/CodexNotch --print-performance-history --limit 200
```

性能历史保存在 `~/Library/Logs/CodexMonitor/performance-samples.jsonl`；指标边界和归属限制见
[`docs/METRIC_DEFINITIONS.md`](docs/METRIC_DEFINITIONS.md) 与 [`docs/DIAGNOSTICS.md`](docs/DIAGNOSTICS.md)。
...
The app currently supports local Codex telemetry, Skill Insights, Codex Radar, CLIProxyAPI / CPA Manager Plus, NewAPI, and Sub2API. It is designed for users who want a persistent, low-friction view of Codex activity, Skill evidence, quota status, model signals, and remote account balances.
...
- Drag the HUD horizontally or vertically to keep its position across launches; use the context menu to return it to the primary-screen center.
- Press the global `⌃⌥⌘M` shortcut to show or hide details without revealing a hidden menu-bar interface.
- Multi-tab detail panel: `Codex`, `Performance`, `Skills`, `Codex Radar`, `CLIProxyAPI`, `NewAPI`, and `Sub2API`.
- The `Performance` tab shows CPU, resident memory, and 30-second peaks for the Codex / ChatGPT process tree, Safari host processes, the hottest WebKit content candidate, and WindowServer.
...
- Available reset-credit count from the existing quota response, plus Today, 7-day, and 30-day standard-API-price equivalents.
- Deterministic 5-hour and 7-day quota pace, projected exhaustion, source, and last-success freshness.

Cost is a local standard-API-price equivalent, not a ChatGPT/Codex subscription invoice; Priority, regional premiums, and tool-call charges are excluded. Initial history scan displays `Backfilling` instead of publishing a partial subtotal, and unknown models are never guessed. Parsing follows CodexBar's first-session-metadata, fork-lineage, and cross-file usage-row deduplication semantics. Each coordinated serial utility request executes at most one 8 MiB / 50ms CPU / 250ms wall slice; later refreshes resume from complete-line checkpoints, and only a caught-up corpus is atomically published in `usage-deltas.sqlite`. Once complete, footer Token and cost values use that same result; existing CLI, Node, and JSON Token fields remain unchanged. Opening details never starts the job, power or thermal constraints pause it, and a warm caught-up refresh rereads no JSONL.

After passing the runtime energy gates, **adaptive low-energy refresh** defaults
to on for new installations. Existing installations keep their recorded choice;
an older installation without one stays on the conservative fixed cadence. When
enabled, local and remote cadence responds to the corresponding detail tab being
expanded, active Codex work, Low Power Mode, and thermal pressure. The collapsed
HUD renders cached state without selecting the foreground cadence. File events,
manual refresh, and quota-reset boundaries remain responsive, and presenting
fresh detail data starts no duplicate scan, network request, or child process.
...
### Personal Pro Official Web Analytics

Below the task table, the Codex detail page includes a 48-point **Official Data ·
7 Days** entry for the current personal Pro account. It shows Turns, Skills,
and Plugin calls from the visible Codex Analytics page. A separate `Analytics`
detail page renders native stacked area charts for Turns (By model / By surface)
and Skills used, with vertical scrolling inside the fixed HUD. Web quality and
login state are isolated from local tasks, quota, Token history, and RUN / IDLE.

Click **Web Sign-in** and authenticate in Codex Monitor's visible browser
window. The login session is retained in the app-owned WebKit website data
store so it can be reused after restart. The app does not reuse Chrome or
Safari cookies and does not persist passwords, raw HTML, or the Analytics
snapshot. **Clear Sign-in** removes only this app's WebKit session without
affecting other browsers. A successful result is cached in memory for 30
minutes and refreshes only when details are presented after expiry or the user
refreshes; there is no background Analytics timer.
App launch, the Codex detail, and Local Token mode do not materialize WebKit.
Only an explicit Official Analytics or Web Sign-in action does so. After both
official surfaces remain unused for 30 minutes, the `WKWebView` is released
while the same website data store is retained for the next app-owned sign-in.
If ChatGPT leaves the login flow on its home page, **Reload Web Page** returns
to the exact official Analytics URL.

The personal Pro page has no stable public Analytics API. Codex Monitor reads
only the three visible KPIs and user-visible model, Surface, and Skills chart
Tooltips. All three charts must cover the same seven dates; model and Surface
totals must equal Turns, while Skill totals must equal Skills used. Verified
partial dates remain visible with `PARTIAL`, but missing dates are never filled
with synthetic zeroes. DOM changes, expired login, missing fields, range
selection failures, or failed totals become `PARTIAL`, `STALE`, or `UNAVAILABLE`
instead of local estimates. The native page does not create another WebKit;
the existing visible browser remains the login and source-verification surface.

The Analytics page also has a **Local Token** mode. It reads only the complete
published local cost snapshot and shows Today, 7-day, or 30-day model trends
and rankings. Selecting the chart adds no JSONL scan, network request, or
WebKit work. Cached input remains a subset of input, and unpriced models still
contribute to Token totals and shares.

### Background Performance Diagnostics

**Background performance monitoring** defaults off. The visible Performance detail samples every five seconds. A hidden detail samples every 60 seconds only when the option is enabled; Low Power Mode or serious/critical thermal pressure slows an allowed sample to five minutes. Enabled background samples write bounded history to `~/Library/Logs/CodexMonitor/performance-samples.jsonl`. The 4 MiB file keeps one rotated backup and stores only timestamps, CPU, RSS, process counts, selected PIDs, and system memory percentage. It never stores URLs, window titles, command arguments, or conversation content.

Lightweight macOS process data cannot reliably map reparented WebKit XPC processes to a Safari tab, so the UI labels the hottest WebKit process as an unverified owner candidate. A controlled tab refresh or close that removes the PID and drops resource use is strong differential evidence. WindowServer CPU is compositor-pressure and jank-risk evidence, not measured FPS; the app does not request Screen Recording or Accessibility permission to manufacture an FPS value.

### Skill Insights

The `Skills` detail tab provides a read-only, low-frequency seven-day report. Deterministic evidence helps show which enabled Skills were actually used, where activation may have been missed or misfired, and whether a disabled Skill has enough demand signal to justify manual retesting.

It can show:

- Enabled and disabled Skill counts plus the approximate context cost of enabled `name + description` catalog entries.
- `DIRECT`, `STRONG`, `INFERRED`, and `SHADOW` evidence, suspected misses and misfires, and candidates replaced by existing capabilities.
- Conservative keep, observe, remain disabled, retest, and restore-candidate recommendations.
- `COMPLETE`, `PARTIAL`, or `UNAVAILABLE` data completeness, last analysis time, and incremental scan performance.
- Read-only Markdown weekly-report and machine-readable JSON exports.

Enabled state and catalog descriptions come from the local Codex app-server's read-only `skills/list` result. This preserves Codex's own `config.toml`, plugin activation, scope, and path-precedence rules instead of counting inactive plugin caches. If the local interface is unavailable, the app falls back to frontmatter scanning, marks the catalog `PARTIAL`, and never presents that fallback as complete truth.

Skill Insights has an independent switch under **Codex Data** settings. Turning it off removes the `Skills` tab, releases the analyzer service and weekly timer, and stops Skill JSONL reads and derived-database writes. Token, Quota, Context, and Delta monitoring remain unchanged.

When enabled, startup only checks a rolling weekly schedule. Automatic analysis runs at most once every seven days, while **Analyze recent 7 days** resumes from persisted complete-line offsets without clearing checkpoints. Realtime Token refreshes, file changes, and watcher events never trigger Skill analysis. Low Power Mode or a serious/critical thermal state defers that week's automatic attempt without adding retry activity. Catalog requests use local stdio only; the feature makes no model, embedding, or external-classification call, sends no Session or Skill data to an external service, and never edits `~/.codex/config.toml` or a Skill file.

The scanner uses one background thread, a chunked JSONL reader, and a persistent SQLite connection. A run is bounded by 2 GiB of logical JSONL reads, 15 seconds of process CPU time, or 30 seconds of wall time. It saves a complete-line checkpoint and reports `PARTIAL` when a bound is reached, then resumes on the next manual or weekly run. Unchanged files are not reread. Enabled-state changes reclassify neutral evidence at report time and do not rescan JSONL.

Derived data is retained for 30 days in `~/Library/Application Support/CodexNotch/skill-observations.sqlite`. The database does not contain complete prompts, assistant responses, reasoning, or complete tool inputs/outputs. Related Session Tokens are whole-Session reference totals only; per-Skill Token attribution is always `UNAVAILABLE`. See [`docs/METRIC_DEFINITIONS.md`](docs/METRIC_DEFINITIONS.md) and [`docs/DIAGNOSTICS.md`](docs/DIAGNOSTICS.md) for exact definitions and privacy boundaries.
...
- `Codex`: local refresh intervals, opt-in background performance monitoring, quota source, task range, and period usage display.
...
They contain quota values, reset timestamps, sources, decision reasons, and
fixed feature status. They never contain task content, arbitrary keyboard input,
account details, or credentials. See
[`docs/DIAGNOSTICS.md`](docs/DIAGNOSTICS.md) for the schema.

Print a one-shot performance snapshot or the latest 200 background samples:
...
Performance history is stored at `~/Library/Logs/CodexMonitor/performance-samples.jsonl`. See
[`docs/METRIC_DEFINITIONS.md`](docs/METRIC_DEFINITIONS.md) and [`docs/DIAGNOSTICS.md`](docs/DIAGNOSTICS.md) for attribution and privacy boundaries.
