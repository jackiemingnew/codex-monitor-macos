...
没有配置本机 token 时，应用会自动读取 `https://codexradar.com/current.json` 公开 summary。有 token 时会读取 `https://codexradar.com/api/v1/current`；token 无效或 API 暂不可用时会明确提示并降级到公开 summary。Radar token 与其他凭证一起保存在设置中选择的统一凭证库，默认使用 macOS Keychain；`CODEXRADAR_API_TOKEN` 仅作为当前进程的临时最高优先级覆盖。启动和后台刷新不会弹出钥匙串授权，需要授权时本次使用 Public，进入 Radar 或 Radar 设置后才允许交互授权。旧版 Application Support token 文件会在首次交互式加载并验证写入后删除。
...
Without a local token, the app reads the public summary at `https://codexradar.com/current.json`. With a token, it reads `https://codexradar.com/api/v1/current`; an invalid token or temporary API failure is reported before falling back to the public summary. Radar tokens share the selected secret vault with the other credentials and use macOS Keychain by default; `CODEXRADAR_API_TOKEN` remains a process-only highest-priority override. Launch and background refreshes never show Keychain authorization UI: when authorization is required, that refresh uses Public until the user opens Radar or its settings. A verified first interactive load migrates and removes the legacy Application Support token file.
