...
设置中的“后台性能监控”默认关闭。开启后即使详情页收起，也会每 5 秒采样一次，并将有界历史写入
`~/Library/Logs/CodexMonitor/performance-samples.jsonl`。日志最多 4 MiB，并保留一个轮转备份；只记录时间、CPU、RSS、进程数、候选 PID 和系统内存比例，不记录 URL、窗口标题、命令参数或对话内容。
...
**Background performance monitoring** defaults off. When enabled, it samples every five seconds even while the detail panel is closed and writes bounded history to `~/Library/Logs/CodexMonitor/performance-samples.jsonl`. The 4 MiB file keeps one rotated backup and stores only timestamps, CPU, RSS, process counts, selected PIDs, and system memory percentage. It never stores URLs, window titles, command arguments, or conversation content.
